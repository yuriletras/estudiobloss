document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        'en': {
            ...
            "view-typography": "View Typography"
        },
        'pt-BR': {
            ...
            "final-phrase-blossom": "É hora de <span class=\"blossom-italic\">florescer</span>.",
            ...
            "view-logo": "Ver Logotipo",
            "view-palette": "Ver Paleta",
            "view-typography": "Ver Tipografia"
        }
    };
    ...
    const updateTranslations = (lang) => {
        document.querySelectorAll('[data-translate]').forEach(element => {
            ...
            if (translation) {
                ...
                if (overlayText) {
                    overlayText.innerHTML = translation;
                } else {
    ...
    if (backToTopBtn) {
        window.addEventListener('scroll', () => {
            if (window.scrollY > 300 && !document.querySelector('#fullscreen-modal.open')) {
    ...
    // ---- LÓGICA DE CARREGAMENTO DE IMAGENS E SPINNER ----
    const portfolioImages = document.querySelectorAll('.portfolio-full-item-image img, .sample-item img');
    portfolioImages.forEach(img => {
        const parent = img.parentElement;
        const existingSpinner = parent.querySelector('.spinner');
        if (existingSpinner) {
            console.log(`Spinner existente encontrado para: ${img.src}`);
        ...
        console.log(`Spinner adicionado para: ${img.src}`);

        const removeSpinner = () => {
            console.log(`Removendo spinner para: ${img.src}`);
            img.classList.add('loaded');
            spinner.classList.remove('active');
            setTimeout(() => {
                if (spinner.parentNode) {
                    spinner.remove();
                    console.log(`Spinner removido do DOM para: ${img.src}`);
                }
            }, 300);
        };

        if (img.complete && img.naturalWidth !== 0) {
            console.log(`Imagem em cache detectada: ${img.src}`);
            removeSpinner();
        } else {
            img.addEventListener('load', () => {
                console.log(`Evento 'load' disparado para: ${img.src}`);
                removeSpinner();
            }, { once: true });
            img.addEventListener('error', () => {
                ...
                if (spinner.parentNode) {
                    spinner.remove();
                    console.log(`Spinner removido devido a erro para: ${img.src}`);
                }
        ...
        setTimeout(() => {
            if (spinner.classList.contains('active') && spinner.parentNode) {
                console.warn(`Fallback: Removendo spinner para ${img.src} após 5 segundos`);
                spinner.classList.remove('active');
                spinner.remove();
            }
        }, 5000);
    ...
    // ---- LÓGICA DO MODAL DO PORTFÓLIO COM ZOOM E PAN ----
    const modal = document.getElementById('fullscreen-modal');
    const modalImage = document.querySelector('.modal-image');
    const modalCloseBtn = document.querySelector('.modal-close-btn');
    const imageOverlays = document.querySelectorAll('.portfolio-full-item-image .image-overlay, .sample-item .image-overlay');
    ...
    if (modal && modalImage && modalCloseBtn && imageOverlays.length > 0) {
        ...
        const existingZoomInBtn = modal.querySelector('.zoom-btn.zoom-in');
        if (existingZoomInBtn) existingZoomInBtn.remove();
        const zoomInBtn = document.createElement('button');
        zoomInBtn.classList.add('zoom-btn', 'zoom-in');
        zoomInBtn.innerHTML = '+';
        zoomInBtn.title = 'Aumentar Zoom';
        modal.appendChild(zoomInBtn);

        const existingZoomOutBtn = modal.querySelector('.zoom-btn.zoom-out');
        if (existingZoomOutBtn) existingZoomOutBtn.remove();
        const zoomOutBtn = document.createElement('button');
        zoomOutBtn.classList.add('zoom-btn', 'zoom-out');
        ...
        imageOverlays.forEach(overlay => {
            overlay.addEventListener('click', () => {
                ...
                if (img && img.tagName === 'IMG') {
                    ...
                    const existingSpinner = modal.querySelector('.spinner');
                    if (existingSpinner) {
                        console.log(`Spinner existente no modal removido para: ${img.src}`);
                        existingSpinner.remove();
                    }
                    ...
                    console.log(`Spinner adicionado ao modal para: ${img.src}`);

                    modalImage.src = img.src;
                    modal.classList.add('open');
                    document.body.classList.add('no-scroll');
                    if (backToTopBtn) {
                        backToTopBtn.classList.remove('show'); // Oculta o botão "Voltar ao Topo"
                    }
                    ...
                    const removeModalSpinner = () => {
                        console.log(`Imagem no modal carregada: ${modalImage.src}`);
                        modalImage.classList.add('loaded');
                        spinner.classList.remove('active');
                        setTimeout(() => {
                            if (spinner.parentNode) {
                                spinner.remove();
                                console.log(`Spinner do modal removido para: ${modalImage.src}`);
                            }
                        }, 300);
                    };

                    if (modalImage.complete && modalImage.naturalWidth !== 0) {
                        console.log(`Imagem no modal em cache: ${modalImage.src}`);
                        removeModalSpinner();
                    } else {
                        modalImage.addEventListener('load', removeModalSpinner, { once: true });
                        modalImage.addEventListener('error', () => {
                            console.error(`Erro ao carregar a imagem no modal: ${modalImage.src}`);
                            spinner.classList.remove('active');
                            if (spinner.parentNode) {
                                spinner.remove();
                                console.log(`Spinner do modal removido devido a erro para: ${modalImage.src}`);
                            }
                    ...
                    setTimeout(() => {
                        if (spinner.classList.contains('active') && spinner.parentNode) {
                            console.warn(`Fallback: Removendo spinner do modal para ${modalImage.src} após 5 segundos`);
                            spinner.classList.remove('active');
                            spinner.remove();
                        }
                    }, 5000);
        ...
        modalCloseBtn.addEventListener('click', () => {
            ...
            if (backToTopBtn && window.scrollY > 300) {
                backToTopBtn.classList.add('show'); // Restaura o botão "Voltar ao Topo" se necessário
            }
        });
